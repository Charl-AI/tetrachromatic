...
def test_mp_conv():
    x0 = torch.zeros(16, 3, 32, 32)
    x1 = torch.ones(16, 3, 32, 32)
    x2 = torch.randn(16, 3, 64, 64)

    kernel_sizes = [1, 3]
    for k in kernel_sizes:
        tconv = torchedm.MPConv(3, 16, kernel=(k, k)).eval()
        jconv = jaxedm.MPConv(3, 16, kernel=(k, k), rngs=nnx.Rngs(0))
        assert tconv.weight.shape == j2t(jconv.weight).shape  # type: ignore
        jconv.weight.value = t2j(tconv.weight)  # model surgery

        for x in [x0, x1, x2]:
            j = jconv(t2j(x))
            t = tconv(x)
            assert torch.allclose(t, j2t(j), rtol=1e-6, atol=1e-5)
